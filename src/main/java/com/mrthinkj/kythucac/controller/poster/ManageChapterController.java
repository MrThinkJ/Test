...
//        }
//        content = sb.toString();
        String status = chapterService.addNewChapter(bookName, chapter
//                content
        );
        return status;
    }
    ...
    public @ResponseBody String updateChapter(@ModelAttribute("userAccount") Account account,
        ...
        if (result.hasErrors()) {
            ...
            if (result.hasFieldErrors("price")){
                return "Giá chương không được để trống hoặc chứa ký tự đặc biệt và chữ số";
            }
            return errors.get(0).getDefaultMessage();
        }
//        String content;
...
//        }
//        content = sb.toString();
        String status = chapterService.updateChapter(bookName, chapter);
        return status;
    }

    @PostMapping("/delete-chapter")
    public @ResponseBody String deleteChapter() {
        String status = chapterService.deleteChapter("Tiên Nghịch");
        return status;
    }
