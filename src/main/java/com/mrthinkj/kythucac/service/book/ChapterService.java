...
public class ChapterService {
    ...
    public String addNewChapter(String bookName, Chapter chapter
//            , String content
    ){
        Book book = convert.findBookByName(bookName);
        Integer chapterId = chapterRepository.findIdByIndexAndBook(chapter.getIndex(), book.getId());
        if (chapterId != null)
            return "Đã có chương truyện có số thứ tự này";
        if (!chapter.isVipStatus())
            chapter.setPrice(0);
        if (chapter.isVipStatus() && chapter.getPrice() <= 0 || chapter.getPrice() > 300)
            return "Chương vip giá tiền phải lớn hơn 0, nhỏ hơn 301";
        chapter.setId(0);
    ...
    public String updateChapter(String bookName, Chapter chapter){
        Book book = convert.findBookByName(bookName);
        Integer chapterId = chapterRepository.findIdByIndexAndBook(chapter.getIndex(), book.getId());
        if (chapterId != null && chapterId != chapter.getIndex())
            return "Đã có chương có số thứ tự này";
        if (!chapter.isVipStatus())
            chapter.setPrice(0);
        if (chapter.getPrice() <= 0 || chapter.getPrice() > 300)
            return "Chương vip giá tiền phải lớn hơn 0, nhỏ hơn 301";
        chapter.setBook(book);
        chapter.setPostDate(LocalDate.now());
        chapterRepository.save(chapter);
    ...
    public String deleteChapter(String bookName){
        Book book = convert.findBookByName(bookName);
        chapterRepository.deleteByBookAndIndex(book, 1);
        return "thanh cong";
    }
}
